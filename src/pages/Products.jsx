...
export function Products() {
    ...
    const { products, error, loading, retry } = useProducts({ category: CATEGORY.ALL.value });
    // 從首頁點卡片來選擇目錄
            ...
            lg:justify-center lg:gap-[100px] lg:mt-[40px]">
                <div className="list-category w-full flex flex-col justify-center items-center lg:max-w-[300px]">
                    <ul className="text-center w-full  flex flex-col justify-center items-center">
                        ...
                        {
                            loading ? (<li className="w-full flex justify-center items-center py-[20px] ">
                                {loadingDom}
                            </li>)
                                : error ? (
                                    <li className="w-full mt-[20px] py-[20px] flex flex-col items-center justify-center gap-2 bg-[#EAF0ED]">
                                        <p className="text-[#E57373] > font-['PingFang TC'] leading-[20px] text-[24px] font-semibold py-[17px] px-[20px]">載入失敗</p>
                                        <button
                                            onClick={retry}
                                            className=" border border-[#8DA291] text-[#8DA291] font-medium px-5 py-2.5 rounded-md hover:bg-[#3F5D45] active:bg-[#3F5D45] hover:text-[#FFFFFF] active:text-[#FFFFFF] transition-colors "
                                        >
                                            重試
                                        </button>
                                    </li>
                    ...
                    lg:grid lg:grid-cols-2 lg:gap-[20px] lg:px-0">
                        {
                            loading ? (<li className="w-full flex justify-center items-center py-[20px] ">
                                ...
                                : error ?
                                    <li className="w-full  flex justify-center items-center py-[20px] ">
